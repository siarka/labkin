...
def det_tags(
    ...
    if wykryte:
        ...
        p_z_1 = stala_cen / oblicz_odleglosc(corner_01, corner_03)
        p_z_2 = stala_cen / oblicz_odleglosc(corner_02, corner_04)
        z_cen = (p_z_1 + p_z_2) / 20
        center_poz = np.array([
       ...
       # ])
        L = np.array([
           [-1/z[0], 0, center_poz[0]/z[0], center_poz[0]*center_poz[1], -1-(center_poz[0]*center_poz[0]), center_poz[1]],
           [0, -1/z[0], center_poz[1]/z[0], 1+(center_poz[1]*center_poz[1]), -center_poz[0]*center_poz[1], -center_poz[0]]
       ])
        pi_L = np.linalg.pinv(L)
        center_poz_kartezjan = kartezja_na_biegunowe(center_poz[0],center_poz[1])

        result = np.dot(pi_L, center_poz)
    
        if z_cen < 60.4 and z_cen > 59.6:
            result[2] = 0.0
        else:
            result[2] = 0.00001*(z_cen-60)

        result *= 200
        result = np.round(result, 4)
        center_poz= np.round(center_poz, 4)
    else:
        z_cen = -1
        err = np.array([0, 0, 0, 0, 0, 0, 0, 0])
        result = np.array([0, 0, 0, 0, 0, 0])
        center_poz_kartezjan = kartezja_na_biegunowe(0,0)
        center_poz = np.array([-1,-1])
    ...
    dt = round(dt, 3)
    z_cen = round(z_cen, 3)
    print(dt,center_poz[0],center_poz[1],z_cen,result[0],result[1],result[2])
    return err
